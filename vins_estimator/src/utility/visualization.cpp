...
void printStatistics(const Estimator &estimator, double timestamp) {
	if (estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
		return;
	static Eigen::Vector3d last_path   = estimator.Ps[WINDOW_SIZE];
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header) {
	...
	if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
		...
		for (auto &_it : estimator.f_manager.feature) {
			...
			// ROS_INFO("START FRAME %d FRAME_SIZE %d WIN SIZE %d solve flag %d", it_per_id.start_frame, frame_size,
			// WINDOW_SIZE, it_per_id.solve_flag);
			if (it_per_id.start_frame < WINDOW_SIZE && it_per_id.start_frame + frame_size >= WINDOW_SIZE &&
				...
				Vector3d w_pts_i = estimator.Rs[imu_j] *
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header) {
	...
	for (auto _it : estimator.f_manager.feature) {
		...
		Vector3d pts_i = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
		Vector3d w_pts_i =
			estimator.Rs[imu_i] * (estimator.ric[it_per_id.main_cam] * pts_i + estimator.tic[it_per_id.main_cam]) +
			estimator.Ps[imu_i];
