...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const FeatureFrame &image, double td) {
	...
	if (frame_count < 2 || last_track_num < 20 || long_track_num < KEYFRAME_LONGTRACK_THRES ||
	...
	if (parallax_num == 0) {
		...
		if (is_kf) {
			ROS_DEBUG(
				"Add KF: Parallax is bigger than required == 0 parallax_sum %f parallax_num %d avg %f MIN_PARALLAX %f",
...
std::map<int, double> FeatureManager::getDepthVector() {
	// This function gives actually points for solving; We only use oldest max_solve_cnt point, oldest pts has good
	// track As for some feature point not solve all the time; we do re triangulate on it
...
void FeatureManager::triangulatePoint3DPts(Eigen::Matrix<double, 3, 4> &Pose0, Eigen::Matrix<double, 3, 4> &Pose1,
										   Eigen::Vector3d &point0, Eigen::Vector3d &point1,
...
double FeatureManager::triangulatePoint3DPts(vector<Eigen::Matrix<double, 3, 4>> &poses,
											 vector<Eigen::Vector3d> &points, Eigen::Vector3d &point_3d) {
	// TODO:Rewrite this for 3d point
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[]) {
	for (auto &_it : feature) {
		...
		// ROS_INFO("Pt3d %f %f %f LocalPt %f %f %f", point3d.x(), point3d.y(), point3d.z(), localPoint.x(),
