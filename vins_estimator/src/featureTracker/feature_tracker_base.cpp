...
std::vector<cv::Point2f> detect_orb_by_region(cv::InputArray _img, cv::InputArray _mask, int features, int cols,
											  int rows) {
...
vector<cv::Point2f> get_predict_pts(vector<int> ids, const vector<cv::Point2f> &cur_pt,
...
bool BaseFeatureTracker::inBorder(const cv::Point2f &pt, cv::Size shape) {
	const int BORDER_SIZE = 1;
	int		  img_x		  = cvRound(pt.x);
	int		  img_y		  = cvRound(pt.y);
	return BORDER_SIZE <= img_x && img_x < shape.width - BORDER_SIZE && BORDER_SIZE <= img_y &&
		   img_y < shape.height - BORDER_SIZE;
}

void BaseFeatureTracker::setup_feature_frame(FeatureFrame &ff, vector<int> ids, vector<cv::Point2f> cur_pts,
											 vector<cv::Point3f> cur_un_pts, vector<cv::Point3f> cur_pts_vel,
											 int camera_id) {
	// ROS_INFO("Setup feature frame pts %ld un pts %ld vel %ld on Camera %d", cur_pts.size(), cur_un_pts.size(),
	// cur_pts_vel.size(), camera_id);
	for (size_t i = 0; i < ids.size(); i++) {
...
vector<cv::Point2f> opticalflow_track(vector<cv::Mat> *cur_pyr, vector<cv::Mat> *prev_pyr,
									  vector<cv::Point2f> &prev_pts, vector<int> &ids, vector<int> &track_cnt,
									  std::set<int> &removed_pts, const std::map<int, cv::Point2f> &prediction_points) {
	if (prev_pts.size() == 0) {
...
vector<cv::Point2f> opticalflow_track(cv::Mat &cur_img, vector<cv::Mat> *cur_pyr, cv::Mat &prev_img,
									  vector<cv::Mat> *prev_pyr, vector<cv::Point2f> &prev_pts, vector<int> &ids,
									  vector<int> &track_cnt, std::set<int> &removed_pts,
									  const std::map<int, cv::Point2f> &prediction_points) {
	if (prev_pts.size() == 0) {
		return vector<cv::Point2f>();
	}
	TicToc		  tic;
	...
	for (int i = 0; i < int(cur_pts.size()); i++) {
		if (status[i] && !BaseFeatureTracker::inBorder(cur_pts[i], cur_img.size())) {
			status[i] = 0;
		}
	}
	reduceVector(prev_pts, status);
...
void BaseFeatureTracker::drawTrackImage(cv::Mat &img, vector<cv::Point2f> pts, vector<int> ids,
										map<int, cv::Point2f> prev_pts, map<int, cv::Point2f> predictions) {
	char idtext[10] = {0};
...
#ifndef WITHOUT_CUDA
vector<cv::Point2f> opticalflow_track(cv::cuda::GpuMat &cur_img, std::vector<cv::cuda::GpuMat> &prev_pyr,
									  vector<cv::Point2f> &prev_pts, vector<int> &ids, vector<int> &track_cnt,
									  std::set<int> &removed_pts, bool is_lr_track,
									  const std::map<int, cv::Point2f> &prediction_points) {
...
void detectPoints(const cv::cuda::GpuMat &img, vector<cv::Point2f> &n_pts, vector<cv::Point2f> &cur_pts,
	...
	if (lack_up_top_pts > require_pts / 4) {
		...
		cv::Ptr<cv::cuda::CornersDetector> detector =
			cv::cuda::createGoodFeaturesToTrackDetector(img.type(), lack_up_top_pts, 0.01, MIN_DIST);
